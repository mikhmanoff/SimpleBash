...
Flags parse_flag(int argc, char *argv[]) {
  ...
  for (; currentFlag != -1;
       currentFlag = getopt_long(argc, argv, "+bevEnstT", long_opt, NULL)) {
    switch (currentFlag) {
      case 'b':
        flags.number_non_blank = true;
        break;
      case 'e':
        flags.mark_endline = true;
...
void cat(int argc, char *argv[], Flags flags, const char *table[static 256]) {
  for (char **filename = &argv[optind], **end = &argv[argc]; filename != end;
    ...
    cat_output(file, flags, table);
    fclose(file);
    optind++;
  }
}

void flag_set_endl(const char *table[static 256]) { table['\n'] = "$\n"; }

void flag_set_tab(const char *table[static 256]) { table['\t'] = "^I"; }
...
int main(int argc, char *argv[]) {
  Flags flags = parse_flag(argc, argv);
  const char *table[256];
  cat_set_symbols(table);
  if (errno) {
    fprintf(stderr, "%s", argv[0]);
  } else {
    if (flags.mark_endline) {
      flag_set_endl(table);
    }
    if (flags.print_non_printable) {
      flag_set_nonprintable(table);
    }
    if (flags.tab) {
      flag_set_tab(table);
    }
